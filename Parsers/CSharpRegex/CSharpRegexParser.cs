...
namespace RefactorScope.Parsers.CSharpRegex
{
    ...
    /// Constrói o Modelo Estrutural sem dependência de AST.
    /// </summary>
    public class CSharpRegexParser : IParserCodigo
    {
        ...
        public ModeloEstrutural Parse(string rootPath)
        {
            ...
            foreach (var file in csFiles)
            {
                ...
                foreach (var tipo in tipos.Where(t => t.DeclaredInFile ==
                             Path.GetRelativePath(rootPath, file)))
                {
                    foreach (var possibleTarget in tipoNames)
                    {
                        ...
                        {
                            var referencia = new ReferenciaInfo(tipo.Name, possibleTarget);
            ...
            // Atualizar referências nos tipos
