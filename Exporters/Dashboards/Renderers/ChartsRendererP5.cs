...
const archRadarSketch = (p) => {
    ...
    p.draw = () => {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            p.endShape(p.CLOSE);
            if(i < 5) {
                p.fill('{{TextMuted}}'); p.noStroke(); p.textSize(9);
                p.text(`${i * 20}%`, 5, -r);
...
""");
        ...
        public string RenderArchitecturalGalaxyEnhanced(CouplingResult coupling)
        {
            ...
            foreach (var m in rankedModules)
            {
                ...
                jsData.Append($"{{ name: '{m.Name}', inst: {F(m.Inst)}, abs: {F(m.Abs)}, fan: {m.FanOut}, col: '{fill}', hl: {highlighted} }},");
            }
            jsData.Append("]");
...
const galaxySketch = (p) => {
    ...
    p.draw = () => {
        ...
        // Zone Fills
        p.noStroke();
        p.fill('rgba(255,140,40,0.020)'); p.rect(margin, margin, plotW, plotH);
        p.fill('rgba(255,180,90,0.030)'); p.triangle(margin, margin, p.width - margin, p.height - margin, margin, p.height - margin);
        p.fill('rgba(255,120,40,0.020)'); p.triangle(margin, margin, p.width - margin, margin, p.width - margin, p.height - margin);

        // Grid & Ticks
        p.stroke('rgba(210,125,48,0.16)'); p.strokeWeight(1);
        p.fill('rgba(235,214,194,0.52)'); p.noStroke(); p.textSize(9);
        [0, 0.25, 0.5, 0.75, 1].forEach(tick => {
            let x = margin + plotW * tick; let y = margin + plotH * (1 - tick);
            p.stroke('rgba(210,125,48,0.16)'); p.line(x, margin, x, p.height - margin); p.line(margin, y, p.width - margin, y);
            p.noStroke(); p.textAlign(p.CENTER, p.TOP); p.text(tick, x, p.height - margin + 8);
            p.textAlign(p.RIGHT, p.CENTER); p.text(tick, margin - 6, y);
        });

        // Main Axes
        p.stroke('rgba(235,150,70,0.38)'); p.strokeWeight(1.5);
        p.line(margin, p.height - margin, p.width - margin, p.height - margin);
        p.line(margin, p.height - margin, margin, margin);

        // Main Sequence
        p.stroke('rgba(255,190,100,0.40)'); p.drawingContext.setLineDash([5, 5]);
        p.line(margin, margin, p.width - margin, p.height - margin);
        p.drawingContext.setLineDash([]);
        
        p.noStroke(); p.fill('#f2e1d2'); p.textSize(11);
        p.textAlign(p.RIGHT, p.TOP); p.text("Instability", p.width - margin, p.height - margin + 25);
        p.textAlign(p.LEFT, p.BOTTOM); p.text("Abstractness", 0, margin - 15);
        ...
        points.forEach(pt => {
            ...
            // Efeito pulsante de "estrela"
            let size = baseSize + p.sin(p.frameCount * 0.05 + pt.fan) * 2;

            p.drawingContext.shadowColor = pt.col; p.drawingContext.shadowBlur = 12;
            p.fill(pt.col); p.stroke('#fff0d6'); p.strokeWeight(1.5);
            p.ellipse(x, y, size * 2);

            p.drawingContext.shadowBlur = 0;
            if(pt.hl) {
                p.fill('#f4e6d8'); p.noStroke(); p.textSize(10); p.textAlign(p.LEFT, p.BOTTOM);
