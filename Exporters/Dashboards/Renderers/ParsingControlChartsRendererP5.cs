...
const radarSketch = (p) => {
    ...
    // Raio levemente menor para garantir que não encoste nas bordas
    const maxRadius = 100;

    p.setup = () => {
        // AQUI ESTÁ O SEGREDO: Canvas mais largo (440) para dar espaço lateral pros textos
    ...
    p.draw = () => {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            p.fill('{{BgColor}}');
            p.strokeWeight(2);
            p.ellipse(p.cos(angle) * r, p.sin(angle) * r, 8, 8);
        }
        ...
        p.text(labels[0], 0, -maxRadius - 20); // Topo
        p.text(labels[2], 0, maxRadius + 20);  // Base
        
        // Offset ajustado de 15 para 12 para ficar visualmente coeso
        p.textAlign(p.LEFT, p.CENTER);
        p.text(labels[1], maxRadius + 12, 0);  // Direita
        
        p.textAlign(p.RIGHT, p.CENTER);
        p.text(labels[3], -maxRadius - 12, 0); // Esquerda
    };
};
new p5(radarSketch);
</script>
...
const heatSketch = (p) => {
    const buckets = [
        { val: {{safe}}, col: '{{SemanticSafe}}' },
        { val: {{moderate}}, col: '{{SemanticComplex}}' },
        { val: {{sparse}}, col: '{{SemanticPattern}}' },
        { val: {{anomaly}}, col: '{{SemanticRecovery}}' },
        { val: {{recoveryDep}}, col: '{{SemanticRisk}}' }
    ];
    const total = {{total}};

    p.setup = () => {
        let w = document.getElementById('p5-heat-container').offsetWidth || 1000;
        let canvas = p.createCanvas(w, 36);
    ...
    p.draw = () => {
        ...
        buckets.forEach(b => {
            ...
            p.fill(b.col);
            p.noStroke();
            p.rect(currentX, 0, w - 2, p.height, 4);
...
const sankeySketch = (p) => {
    ...
    const links = [
        { src: 0, tgt: 1, val: {{safeFiles}},                                                   col: '{{SemanticSafe}}',     offset: 0.1 },
        { src: 0, tgt: 2, val: {{complexFiles}},                                                col: '{{SemanticComplex}}',  offset: 0.3 },
        { src: 0, tgt: 3, val: {{astPatternFiles}},                                             col: '{{SemanticPattern}}',  offset: 0.5 },
        { src: 0, tgt: 4, val: {{recoveryFiles}},                                               col: '{{SemanticRecovery}}', offset: 0.7 },
        { src: 1, tgt: 5, val: {{Math.Min(safeFiles + astPatternFiles, extractedTypes)}},      col: '{{SemanticSafe}}',     offset: 0.2 },
        { src: 2, tgt: 6, val: {{Math.Min(complexFiles + recoveryFiles, extractedReferences)}}, col: '{{SemanticComplex}}',  offset: 0.4 },
        { src: 3, tgt: 5, val: {{Math.Min(astPatternFiles, extractedTypes)}},                   col: '{{SemanticPattern}}',  offset: 0.6 },
        { src: 4, tgt: 7, val: {{Math.Max(1, riskFiles)}},                                      col: '{{SemanticRecovery}}', offset: 0.8 },
        { src: 5, tgt: 7, val: {{Math.Max(1, Math.Min(riskFiles, extractedTypes / 5))}},       col: '{{AccentHot}}',        offset: 0.9 },
        { src: 6, tgt: 7, val: {{Math.Max(1, Math.Min(riskFiles, extractedReferences / 10))}}, col: '{{SemanticRisk}}',     offset: 0.55 }
    ];
